...
const GroupModal: React.FC<GroupModalProps> = ({ sheet, onClose, onSave }) => {
    ...
    // FIX: Explicitly type the program state to match the GradeSheet interface.
    const [program, setProgram] = useState<GradeSheet['program']>(sheet?.program || '');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const baseData = {
            ...
            selectedTitle: selectedTitle || 'Untitled Project',
            // FIX: Use the correctly typed program state. The cast is no longer needed.
            program: program,
            date: date || 'Not Set',
            venue: venue || 'Not Set',
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                     ...
                     <div>
                        ...
                        {/* FIX: Cast the select value to the appropriate type on change. */}
                        <select value={program} onChange={e => setProgram(e.target.value as GradeSheet['program'])} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500">
