        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-white shadow-lg rounded-lg p-8 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" value={isPanel1 ? localDetails.title : sheet.selectedTitle} onChange={e => handleDetailChange('title', e.target.value)} disabled={!isPanel1 || areDetailsSet || isSaving} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100"/>
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-gray-700">Program:</label>
                        <select value={isPanel1 ? localDetails.program : sheet.program} onChange={e => handleDetailChange('program', e.target.value)} disabled={!isPanel1 || areDetailsSet || isSaving} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" value={isPanel1 ? localDetails.date : sheet.date} onChange={e => handleDetailChange('date', e.target.value)} disabled={!isPanel1 || areDetailsSet || isSaving} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Venue:</label>
                        <select value={isPanel1 ? localDetails.venue : sheet.venue} onChange={e => handleDetailChange('venue', e.target.value)} disabled={!isPanel1 || areDetailsSet || isSaving} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100">
                ...
                {isPanel1 && !areDetailsSet && (
                     <div className="text-right mt-4">
                        <button onClick={handleSaveDetails} disabled={!canSaveDetails || isSaving} className="px-6 py-2 bg-green-700 text-white rounded-md hover:bg-green-800 disabled:opacity-50">
            ...
            <>
                                ...
                                <tr key={item.id} className="border-b">
                                    ...
                                    <td className="p-3 align-middle w-32">
                                        <div className="flex items-center justify-center space-x-2">
                                            <input type="number" value={grades.titleDefenseScores[item.id] ?? ''} placeholder="0" onChange={e => handleScoreChange(item.id, e.target.value, item.weight)} disabled={isReadOnly || isSaving} className="w-20 p-2 border rounded-md text-center disabled:bg-gray-100"/>
                                    ...
                                    <tr key={item.id} className="border-b">
                                        ...
                                        {sheet.proponents.map(student => (
                                            <td key={student.id} className="p-3 align-middle">
                                                <div className="flex items-center justify-center space-x-2">
                                                    <input type="number" value={grades.individualScores[student.id]?.[item.id] ?? ''} placeholder="0" onChange={e => handleScoreChange(item.id, e.target.value, item.weight, student.id)} disabled={isReadOnly || isSaving} className="w-20 p-2 border rounded-md text-center disabled:bg-gray-100"/>
                ...
                <div className="bg-white shadow-lg rounded-lg p-8 mb-8">
                    ...
                    <textarea value={grades.comments} onChange={e => setGrades(g => g ? { ...g, comments: e.target.value } : null)} disabled={isReadOnly || isSaving} rows={6} className="w-full p-3 border rounded-md disabled:bg-gray-100" placeholder="Enter your overall comments for the group here..."/>
                </div>

                {/* Actions */}
                {!isReadOnly && (
                    <div className="flex justify-end">
                        <button onClick={handleSubmit} disabled={isSaving} className="px-8 py-3 bg-green-700 text-white font-bold rounded-md hover:bg-green-800 disabled:opacity-50">
                            {isSaving ? 'Submitting...' : 'Submit Final Grades'}
                        </button>
                    </div>
                )}
            </>
